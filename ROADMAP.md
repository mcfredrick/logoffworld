...
- [ ] Register `logoff.world` via Cloudflare Registrar + point to Pages
- [ ] Ko-fi donations — create page + membership tier, replace placeholder URLs in footer
- [ ] Fix "Suggest a prompt" — create Tally form, replace `https://tally.so/r/placeholder` in footer
- [ ] Update `data-domain="logoff.world"` in Plausible `<script>` tag once custom domain is live
...
- [ ] Encourage recurring donations — Ko-fi's membership tier handles this natively; ensure tier is visible on the Ko-fi page
...
- [ ] App-as-a-service for charities — same engine, charity-branded, same 80/20 split; use logoff-world as the proof of concept in the pitch
- [ ] Transparency dashboard — static page updated monthly: funds raised, donated, charity impact (can be hand-updated to start)
- [ ] Patron badge — lightweight: Ko-fi webhook sets a signed cookie or short-lived token; badge shown in UI
...
- [ ] Bespoke social engagement apps — use logoff-world as portfolio piece for white-label pitches
